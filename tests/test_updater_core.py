...
def test_check_for_updates_newer(mock_urlopen):
    # Mocking a response with v1.5.0 when current is v1.4.1
    mock_response = MagicMock()
    mock_response.status = 200
    mock_response.read.return_value = b'{"tag_name": "v1.5.0", "body": "New features", "assets": []}'
    mock_response.__enter__.return_value = mock_response
    mock_urlopen.return_value = mock_response

    result = check_for_updates("v1.4.1")
    assert result is not None
    assert result["version"] == "v1.5.0"

@patch("core.updater.urllib.request.urlopen")
def test_check_for_updates_older(mock_urlopen):
    # Mocking v1.3.0 when current is v1.4.1 (no update expected)
    mock_response = MagicMock()
    mock_response.status = 200
